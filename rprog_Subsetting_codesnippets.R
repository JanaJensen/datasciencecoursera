...
# [[ extracts element of list or data frame by index or name with whatever type
#    the element is
# $  extracts element of list or data frame by name with whatever type the 
#    element is
...
x$name         # variable doesn't get dereferenced, no column "name"
               # , so no index, so no result
...
x$a    # only one element starts with a so it knows which you want and returns
       # aardvark's sequence
x[["a"]]  # in this case, it assumes literal string, there is no a, returns null
...
good <- complete.cases(airquality)  # complete.cases omits rows w/partial data
