...
# Assignment: Programming Assignment 3 - Part 3
# Student:    Jeanne-Anne Jensen, jana.jensen@comcast.net
#############################

rankhospital <- function(state,    # 2-char abbr
    ...
    # order data by value (omit missing), then alphabetically by name
    ordData <- stateData[order(stateData$value,stateData$hospital,na.last = NA),]

    # handle "best"/"worst"
    if(num=="best")  num <- 1             # best is listed first
    if(num=="worst") num <- nrow(ordData) # worst is listed last, i.e. same as number of rows
    
    ############
    ## Return hospital name in that state with the given rank 30-day death rate
    ############
    ordData[num,1]  # return just one hospital name, NA if num>nrow
